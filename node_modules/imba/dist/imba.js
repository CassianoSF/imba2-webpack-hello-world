...
class ImbaElement extends HTMLElement {
	...
	slot$(name,ctx){
		...
		return (slots_ = this.__slots)[name] || (slots_[name] = imba.createLiveFragment());
	}
...
var {Document,Node,Text,Comment,Element,SVGElement,HTMLElement,DocumentFragment,Event,CustomEvent,MouseEvent,document} = window;
...
function getIntersectionObserver(){
	var intersect_;
	return (intersect_ = observers.intersect) || (observers.intersect = new IntersectionObserver(
		callback('intersect'),
		{threshold: [0,1]}
	));
};
...
Element.prototype.on$intersect = function(mods,context) {
	...
	if (mods.options) {
		...
		if ((typeof (root_ = opts.root)=='string'||root_ instanceof String)) {
